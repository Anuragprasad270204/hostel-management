  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        "http://hostel-management-3x2z.onrender.com/api/auth/login",
        {
          method: "POST",
          headers: {
      ...
      console.log(
        "[Login] LocalStorage content after set:",
      ...
      if (data.user.role === "student") {
        ...
        if (profileCheckResponse.status === 404) {
          ...
          setError(
            profileErrorData.message || "Failed to verify profile status."
          );
        } else {
      ...
      console.log(
        "[Login] LocalStorage content on catch:",
        localStorage.getItem("token"),
        localStorage.getItem("userRole")
      );
    }
